...
import { Flashcard, processReview } from "@/lib/srs";
...
export default function Lessons() {
  ...
  const handleNext = (knew: boolean) => {
    const updated = processReview(card, knew);
    updateCard(updated);
          ...
          <CardContent className="flex-1 flex flex-col items-center justify-center p-8 text-center h-full">
            <div className="flex-1 flex flex-col items-center justify-center">
              {flipped && card.reading && (
                <div className="text-xl text-muted-foreground mb-4 font-serif">{card.reading}</div>
              )}
              <div className="text-6xl md:text-8xl font-serif mb-8 text-foreground">{card.japanese}</div>
              
              {flipped && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-2">
                  <div className="text-3xl font-semibold text-primary">{card.english}</div>
                  {card.partOfSpeech && (
                    <div className="text-sm text-muted-foreground uppercase tracking-widest">{card.partOfSpeech}</div>
                  )}
                </div>
              )}
            </div>
